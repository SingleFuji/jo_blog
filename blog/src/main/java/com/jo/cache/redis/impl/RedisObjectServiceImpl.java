...
public class RedisObjectServiceImpl implements RedisObjectService
{
	...
	private static final String JO_BLOG = "com.jo.blog";
...
//		logger.info("putObject data:" + obj.toString());
		redisTemplate.execute(new RedisCallback<Object>()
		{
			...
			{
				byte[] btyKey = redisTemplate.getStringSerializer().serialize(JO_BLOG + key);
...
//		logger.info("deleteObject key:" + key);
		redisTemplate.execute(new RedisCallback<Object>()
		{
			...
			{
				connection.del(redisTemplate.getStringSerializer().serialize(JO_BLOG + key));
				return null;
			}
		});
	}
